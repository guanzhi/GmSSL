...
During normal operation OpenSSL (libgmcrypto) will allocate various resources at
...
libgmcrypto (see also L<OPENSSL_init_ssl(3)> for the libgmssl
equivalent).
...
The B<opts> parameter specifies which aspects of libgmcrypto should be
...
Suppress automatic loading of the libgmcrypto error strings. This option is
...
Automatic loading of the libgmcrypto error strings. With this option the
library will automatically load the libgmcrypto error strings.
...
libgmcrypto ciphers. This option is a default option. Once selected subsequent
...
With this option the library will automatically load and make available all
libgmcrypto digests. This option is a default option. Once selected subsequent
calls to OPENSSL_init_crypto() with the option
B<OPENSSL_INIT_NO_ADD_ALL_CIPHERS> will be ignored.

=item OPENSSL_INIT_NO_ADD_ALL_CIPHERS

With this option the library will suppress automatic loading of libgmcrypto
...
With this option the library with automatically initialise the libgmcrypto async
...
The OPENSSL_cleanup() function deinitialises OpenSSL (both libgmcrypto
and libgmssl). All resources allocated by OpenSSL are freed. Typically there
...
the libgmcrypto32.dll entry point. Some windows functions may cause threads to exit
