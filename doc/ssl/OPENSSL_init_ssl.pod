...
OPENSSL_init_ssl - OpenSSL (libgmssl and libgmcrypto) initialisation
...
During normal operation OpenSSL (libgmssl and libgmcrypto) will allocate various
...
this function will explicitly initialise BOTH libgmcrypto and libgmssl. To
explicitly initialise ONLY libgmcrypto see the
...
The B<opts> parameter specifies which aspects of libgmssl and libgmcrypto should be
initialised. Valid options for libgmcrypto are described on the
L<OPENSSL_init_crypto(3)> page. In addition to any libgmcrypto
specific option the following libgmssl options can also be used:

=over 4

=item OPENSSL_INIT_NO_LOAD_SSL_STRINGS

Suppress automatic loading of the libgmssl error strings. This option is
...
Automatic loading of the libgmssl error strings. This option is a
