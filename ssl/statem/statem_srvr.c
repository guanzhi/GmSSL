...
#endif
#ifndef OPENSSL_NO_GMTLS
	// if this is sm2 sign, we need to compute Z from pkey
	// get auth method from SSL *
	// get id from X.509
#endif
...
MSG_PROCESS_RETURN tls_process_client_key_exchange(SSL *s, PACKET *pkt)
{
    ...
    if (alg_k & SSL_kPSK) {
        ...
        if (!tls_process_cke_ecdhe(s, pkt, &al))
            goto err;
#ifndef OPENSSL_NO_GMTLS
    } else if (alg_k & (SSL_kSM2DHE | SSL_kSM2PSK)) {
         if (!tls_process_cke_ecdhe(s, pkt, &al))
             goto err;
/*
    } else if (alg_k & SSL_kSM9DHE) {
         if (!tls_process_cke_sm9dhe(s, pkt, &al))
             goto err;
    } else if (alg_k & SSL_kSM2) {
         if (!tls_process_cke_sm2(s, pkt, &al))
             goto err;
    } else if (alg_k & SSL_kSM9) {
         if (!tls_process_cke_sm9(s, pkt, &al))
             goto err;
*/
#endif
