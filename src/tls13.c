...
/*
int tls13_record_print(FILE *fp, const uint8_t *record,  size_t recordlen, int
format, int indent)
{
        // 目前只支持TLCP的ECC公钥加密套件，因此不论用哪个套件解析都是一样的
        // 如果未来支持ECDHE套件，可以将函数改为宏，直接传入 (conn->cipher_suite
<< 8) format |= tls13_ciphers[0] << 8; return tls_record_print(fp, record,
recordlen, format, indent);
}
*/
...
int tls13_gcm_encrypt(const BLOCK_CIPHER_KEY *key, const uint8_t iv[12],
                      ...
                      size_t padding_len,  // TLSInnerPlaintext.content
                      uint8_t *out,
                      size_t *outlen)  // TLSCiphertext.encrypted_record
{
    uint8_t nonce[12];
    uint8_t aad[5];
    uint8_t *gmac;
    ...
    gmac = out + mlen;
    if (gcm_encrypt(key, nonce, sizeof(nonce), aad, sizeof(aad), mbuf, mlen,
                    out, 16, gmac) != 1) {
        error_print();
        free(mbuf);
        return -1;
...
int tls13_gcm_decrypt(const BLOCK_CIPHER_KEY *key, const uint8_t iv[12],
    ...
    if (gcm_decrypt(key, nonce, 12, aad, 5, in, mlen, gmac, GHASH_SIZE, out) !=
        1) {
        error_print();
        return -1;
    }
...
int tls13_record_encrypt(const BLOCK_CIPHER_KEY *key, const uint8_t iv[12],
                         const uint8_t seq_num[8], const uint8_t *record,
                         size_t recordlen, size_t padding_len,
                         uint8_t *enced_record, size_t *enced_recordlen) {
    // 被加密的是握手消息或者是应用层数据

    if (tls13_gcm_encrypt(key, iv, seq_num, record[0], record + 5,
                          recordlen - 5, padding_len, enced_record + 5,
                          enced_recordlen) != 1) {
        error_print();
        return -1;
    }
...
int tls13_record_decrypt(const BLOCK_CIPHER_KEY *key, const uint8_t iv[12],
                         const uint8_t seq_num[8], const uint8_t *enced_record,
                         size_t enced_recordlen, uint8_t *record,
                         size_t *recordlen) {
    int record_type;

    if (tls13_gcm_decrypt(key, iv, seq_num, enced_record + 5,
                          enced_recordlen - 5, &record_type, record + 5,
                          recordlen) != 1) {
        error_print();
        return -1;
    }
...
int tls13_send(TLS_CONNECT *conn, const uint8_t *data, size_t datalen,
               size_t *sentlen) {
    ...
    if (tls13_gcm_encrypt(key, iv, seq_num, TLS_record_application_data, data,
                          datalen, padding_len, record + 5, &recordlen) != 1) {
        error_print();
        return -1;
    }
...
int tls13_do_recv(TLS_CONNECT *conn) {
    ...
    if (tls13_gcm_decrypt(key, iv, seq_num, record + 5, recordlen - 5,
...
int tls13_recv(TLS_CONNECT *conn, uint8_t *out, size_t outlen,
               size_t *recvlen) {
...
int tls13_hkdf_extract(const DIGEST *digest, const uint8_t salt[32],
...
int tls13_hkdf_expand_label(const DIGEST *digest, const uint8_t secret[32],
                            ...
                            size_t context_len, size_t outlen, uint8_t *out) {
    uint8_t label_len;
    uint8_t hkdf_label[2 + 256 + 256];
    uint8_t *p = hkdf_label;
    size_t hkdf_label_len = 0;

    label_len = (uint8_t)(strlen("tls13 ") +
                          strlen(label));  // FIXME: check length < 255
    tls_uint16_to_bytes((uint16_t)outlen, &p, &hkdf_label_len);
    tls_uint8_to_bytes(label_len, &p, &hkdf_label_len);
    tls_array_to_bytes((uint8_t *)"tls13 ", strlen("tls13 "), &p,
                       &hkdf_label_len);
...
int tls13_derive_secret(const uint8_t secret[32], const char *label,
                        const DIGEST_CTX *dgst_ctx, uint8_t out[32]) {
    DIGEST_CTX ctx = *dgst_ctx;
    uint8_t dgst[64];
    size_t dgstlen;

    if (digest_finish(&ctx, dgst, &dgstlen) != 1 ||
        tls13_hkdf_expand_label(dgst_ctx->digest, secret, label, dgst, 32,
                                dgstlen, out) != 1) {
        error_print();
        return -1;
    }
...
static const uint8_t TLS13_client_context_str_and_zero[] =
    "TLS 1.3, client CertificateVerify";
static const uint8_t TLS13_server_context_str_and_zero[] =
    "TLS 1.3, server CertificateVerify";
static size_t TLS13_client_context_str_and_zero_size =
    sizeof(TLS13_client_context_str_and_zero);
static size_t TLS13_server_context_str_and_zero_size =
    sizeof(TLS13_server_context_str_and_zero);

int tls13_sign_certificate_verify(int tls_mode, const SM2_KEY *key,
                                  const char *signer_id, size_t signer_id_len,
                                  const DIGEST_CTX *tbs_dgst_ctx, uint8_t *sig,
                                  size_t *siglen) {
...
int tls13_verify_certificate_verify(int tls_mode, const SM2_KEY *public_key,
                                    const char *signer_id, size_t signer_id_len,
    ...
    sm2_verify_update(&verify_ctx, context_str_and_zero,
                      context_str_and_zero_len);
...
int tls13_compute_verify_data(const uint8_t *handshake_traffic_secret,
                              const DIGEST_CTX *dgst_ctx, uint8_t *verify_data,
    ...
    tls13_hkdf_expand_label(dgst_ctx->digest, handshake_traffic_secret,
                            "finished", NULL, 0, finished_key_len,
                            finished_key);

    hmac(dgst_ctx->digest, finished_key, finished_key_len, dgst, dgstlen,
...
int tls13_client_hello_exts_set(uint8_t *exts, size_t *extslen, size_t maxlen,
    ...
    *extslen = 0;
    if (tls13_supported_versions_ext_to_bytes(
            TLS_client_mode, protocols, protocols_cnt, NULL, extslen) != 1 ||
        tls_supported_groups_ext_to_bytes(
            supported_groups, supported_groups_cnt, NULL, extslen) != 1 ||
        tls_signature_algorithms_ext_to_bytes(sig_algs, sig_algs_cnt, NULL,
                                              extslen) != 1 ||
        tls13_client_key_share_ext_to_bytes(client_ecdhe_public, NULL,
                                            extslen) != 1) {
        error_print();
        return -1;
    }
    ...
    *extslen = 0;
    tls13_supported_versions_ext_to_bytes(TLS_client_mode, protocols,
                                          protocols_cnt, &exts, extslen);
    tls_supported_groups_ext_to_bytes(supported_groups, supported_groups_cnt,
                                      &exts, extslen);
    tls_signature_algorithms_ext_to_bytes(sig_algs, sig_algs_cnt, &exts,
                                          extslen);
...
int tls13_process_client_hello_exts(const uint8_t *exts, size_t extslen,
                                    const SM2_KEY *server_ecdhe_key,
                                    SM2_POINT *client_ecdhe_public,
                                    uint8_t *server_exts,
                                    size_t *server_exts_len,
                                    size_t server_exts_maxlen) {
    size_t len = 0;
    *server_exts_len = 0;
    ...
    while (extslen) {
        ...
        if (tls_uint16_from_bytes(&ext_type, &exts, &extslen) != 1 ||
            tls_uint16array_from_bytes(&ext_data, &ext_datalen, &exts,
                                       &extslen) != 1) {
            error_print();
            return -1;
        }
        ...
        switch (ext_type) {
            /*
            // tls13_process_client_hello_exts
            的接口需要处理，部分输出要输出到server_exts中 case
            TLS_extension_supported_groups: // 这个应该放在EE里面 if
            (tls_process_client_supported_groups(ext_data, ext_datalen, NULL,
            &len) !=
            1
                    ...
                    tls_process_client_supported_groups(ext_data, ext_datalen,
            &server_exts, server_exts_len); break; case
            TLS_extension_signature_algorithms: //
            client单方面通知就可以了，服务器不需要响应 if
            (tls_process_client_signature_algorithms(ext_data, ext_datalen,
            NULL, &len)
            != 1
                            || len > server_exts_maxlen) {
                            error_print();
                    ...
                    tls_process_client_signature_algorithms(ext_data,
            ext_datalen, &server_exts, server_exts_len); break;
            */
            case TLS_extension_supported_versions:
                if (tls13_process_client_supported_versions(
                        ext_data, ext_datalen, NULL, &len) != 1 ||
                    len > server_exts_maxlen) {
                    error_print();
                    return -1;
                }
                tls13_process_client_supported_versions(
                    ext_data, ext_datalen, &server_exts, server_exts_len);
                break;
            case TLS_extension_key_share:
                if (tls13_process_client_key_share(
                        ext_data, ext_datalen, server_ecdhe_key,
                        client_ecdhe_public, &server_exts,
                        server_exts_len) != 1 ||
                    len > server_exts_maxlen) {
                    error_print();
                    return -1;
            ...
            default:;  // server ignore unkonwn extensions
        }
    }
...
int tls_client_key_shares_from_bytes(SM2_POINT *sm2_point, const uint8_t **in,
                                     size_t *inlen) {
    ...
    while (key_shares_len) {
        ...
        tls_uint16array_from_bytes(&key_exch, &key_exch_len, &key_shares,
                                   &key_shares_len);
...
int tls13_server_hello_extensions_get(const uint8_t *exts, size_t extslen,
    ...
    while (extslen) {
        ...
        switch (ext_type) {
            case TLS_extension_supported_versions:
                if (tls_uint16_from_bytes(&version, &ext_data, &ext_datalen) !=
                        1 ||
                    ext_datalen > 0) {
                    error_print();
                    return -1;
                ...
                break;
            case TLS_extension_key_share:
                if (tls13_process_server_key_share(ext_data, ext_datalen,
                                                   sm2_point) != 1) {
                    error_print();
                    return -1;
                }
...
int tls13_encrypted_extensions_print(FILE *fp, int fmt, int ind,
    ...
    if (exts) {
        tls13_extensions_print(fp, fmt, ind, TLS_handshake_encrypted_extensions,
                               exts, extslen);
    }
...
int tls13_record_set_handshake_encrypted_extensions(uint8_t *record,
                                                    size_t *recordlen) {
    ...
    tls_supported_groups_ext_to_bytes(supported_groups,
                                      sizeof(supported_groups) / sizeof(int),
                                      &pexts, &extslen);
...
/*
   ...
   列出客户端支持的签名+哈希算法 ServerHello.Extensions.supported_groups
   决定了服务器的公钥类型， 因此也决定了服务器的签名算法
...
注意：TLS 1.2中只有RAW signature,
也就是没有经过uint16array封装的，这其实不太符合TLS的设计逻辑
*/
int tls13_record_set_handshake_certificate_verify(uint8_t *record,
                                                  size_t *recordlen,
                                                  int sign_algor,
                                                  const uint8_t *sig,
                                                  size_t siglen) {
...
int tls13_record_set_handshake_certificate_request(
    uint8_t *record, size_t *recordlen, const uint8_t *request_context,
    size_t request_context_len, const uint8_t *exts, size_t extslen) {
    ...
    tls_uint8array_to_bytes(request_context, request_context_len, &data,
                            &datalen);
...
int tls13_record_set_handshake_certificate_request_default(uint8_t *record,
                                                           size_t *recordlen) {
    int sig_algs[] = {TLS_sig_sm2sig_sm3};
    uint8_t exts[256];
    uint8_t *p = exts;
    ...
    tls_signature_algorithms_ext_to_bytes(
        sig_algs, sizeof(sig_algs) / sizeof(int), &p, &extslen);
    tls13_record_set_handshake_certificate_request(record, recordlen, NULL, 0,
                                                   exts, extslen);
    return 1;
}

int tls13_record_get_handshake_certificate_request(
    const uint8_t *record, const uint8_t **requst_context,
    size_t *request_context_len, const uint8_t **exts, size_t *exts_len) {
    int type;
    const uint8_t *p;
    size_t len;
    ...
    if (tls_uint8array_from_bytes(requst_context, request_context_len, &p,
                                  &len) != 1 ||
...
TLS 1.3可以只传公钥不传证书 case X509: opaque cert_data<1..2^24-1>;
        };
                Extension extensions<0..2^16-1>;
} CertificateEntry;
...
struct {
        opaque certificate_request_context<0..2^8-1>; --
用于客户端证书，服务器证书该域长度为0 CertificateEntry
certificate_list<0..2^24-1>; } Certificate;
...
int tls13_certificate_print(FILE *fp, int fmt, int ind, const uint8_t *cert,
                            size_t certlen) {
    const uint8_t *p;
    size_t len;
    ...
    while (len) {
        ...
        if (tls_uint24array_from_bytes(&cert_data, &cert_data_len, &p, &len) !=
                1 ||
        ...
        x509_cert_print(fp, fmt, ind + 4, "Certificate", cert_data,
                        cert_data_len);
        x509_cert_to_pem(cert_data, cert_data_len, fp);
        tls13_extensions_print(fp, fmt, ind + 4, TLS_handshake_certificate,
                               exts, extslen);
    }
    return 1;
}

int tls13_certificate_request_print(FILE *fp, int fmt, int ind,
                                    const uint8_t *certreq, size_t certreqlen) {
    const uint8_t *p;
    size_t len;
...
int tls13_certificate_verify_print(FILE *fp, int fmt, int ind, const uint8_t *d,
                                   size_t dlen) {
    uint16_t sig_alg;
    const uint8_t *sig;
    size_t siglen;
...
int tls13_certificate_list_to_bytes(const uint8_t *certs, size_t certslen,
    ...
    while (certslen) {
        ...
        tls_uint16array_to_bytes(entry_exts, entry_exts_len, &p,
                                 &cert_list_len);
    }
...
int tls13_process_certificate_list(const uint8_t *cert_list,
                                   size_t cert_list_len, uint8_t *certs,
                                   size_t *certs_len) {
    *certs_len = 0;
    ...
    while (cert_list_len) {
        ...
        if (tls_uint24array_from_bytes(&cert_data, &cert_data_len, &cert_list,
                                       &cert_list_len) != 1 ||
            tls_uint16array_from_bytes(&exts, &exts_len, &cert_list,
                                       &cert_list_len) != 1) {
            error_print();
            return -1;
        }
        if (x509_cert_from_der(&cert, &cert_len, &cert_data, &cert_data_len) !=
                1 ||
        ...
        while (exts_len) {
            ...
            if (tls_ext_from_bytes(&ext_type, &ext_data, &ext_data_len, &exts,
                                   &exts_len) != 1) {
                error_print();
                return -1;
            }
...
int tls13_record_set_handshake_certificate(uint8_t *record, size_t *recordlen,
    ...
    datalen = 0;
    tls_uint8array_to_bytes(request_context, request_context_len, NULL,
                            &datalen);
...
int tls13_record_get_handshake_certificate(const uint8_t *record,
                                           const uint8_t **cert_request_context,
                                           size_t *cert_request_context_len,
    ...
    if (tls_uint8array_from_bytes(cert_request_context,
                                  cert_request_context_len, &p, &len) != 1 ||
...
verify_data = HMAC(finished_key, Hash(Handshake Context, Certificate*,
CertificateVerify*)) Hash = SM3, SHA256 or SHA384
*/
...
int tls13_cipher_suite_get(int cipher_suite, const DIGEST **digest,
...
[14]         +-----> Derive-Secret(., "res master",
                                                                   ClientHello...client
Finished) = resumption_master_secret
...
int tls13_do_connect(TLS_CONNECT *conn) {
    ...
    tls13_client_hello_exts_set(client_exts, &client_exts_len,
                                sizeof(client_exts),
                                &(client_ecdhe.public_key));
    tls_record_set_handshake_client_hello(
        record, &recordlen, TLS_protocol_tls12, client_random, NULL, 0,
    ...
    if (tls_record_get_handshake_server_hello(
            enced_record, &protocol, &random, &session_id, &session_id_len,
    ...
    if (tls_cipher_suite_in_list(
            cipher_suite, tls13_ciphers,
    ...
    if (tls13_server_hello_extensions_get(server_exts, server_exts_len,
    ...
    */
    ...
    /* [5]  */ tls13_derive_secret(early_secret, "derived", &null_dgst_ctx,
                                   handshake_secret);
    /* [6]  */ tls13_hkdf_extract(digest, handshake_secret,
                                  (uint8_t *)&server_ecdhe_public,
                                  handshake_secret);
    /* [7]  */ tls13_derive_secret(handshake_secret, "c hs traffic", &dgst_ctx,
                                   client_handshake_traffic_secret);
    /* [8]  */ tls13_derive_secret(handshake_secret, "s hs traffic", &dgst_ctx,
                                   server_handshake_traffic_secret);
    /* [9]  */ tls13_derive_secret(handshake_secret, "derived", &null_dgst_ctx,
                                   master_secret);
    /* [10] */ tls13_hkdf_extract(digest, master_secret, zeros, master_secret);
    //[sender]_write_key = HKDF-Expand-Label(Secret, "key", "", key_length)
    //[sender]_write_iv  = HKDF-Expand-Label(Secret, "iv", "", iv_length)
    //[sender] in {server, client}
    tls13_hkdf_expand_label(digest, server_handshake_traffic_secret, "key",
                            NULL, 0, 16, server_write_key);
    tls13_hkdf_expand_label(digest, server_handshake_traffic_secret, "iv", NULL,
                            0, 12, conn->server_write_iv);
    block_cipher_set_encrypt_key(&conn->server_write_key, cipher,
                                 server_write_key);
    memset(conn->server_seq_num, 0, 8);
    tls13_hkdf_expand_label(digest, client_handshake_traffic_secret, "key",
                            NULL, 0, 16, client_write_key);
    tls13_hkdf_expand_label(digest, client_handshake_traffic_secret, "iv", NULL,
                            0, 12, conn->client_write_iv);
    block_cipher_set_encrypt_key(&conn->client_write_key, cipher,
                                 client_write_key);
    ...
    */
    ...
    if (tls13_record_decrypt(&conn->server_write_key, conn->server_write_iv,
                             conn->server_seq_num, enced_record,
                             enced_recordlen, record, &recordlen) != 1) {
        error_print();
    ...
    if (type == TLS_handshake_certificate_request) {
        ...
        if (tls13_record_get_handshake_certificate_request(
                record, &request_context, &request_context_len,
    ...
    tls13_record_trace(stderr, record, recordlen, 0, 0);
    if (tls13_record_get_handshake_certificate(record, &request_context,
                                               &request_context_len, &cert_list,
                                               &cert_list_len) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_unexpected_message);
        return -1;
    ...
    if (x509_certs_get_cert_by_index(conn->server_certs, conn->server_certs_len,
                                     0, &cert, &certlen) != 1 ||
        x509_cert_get_subject_public_key(cert, certlen, &server_sign_key) !=
            1) {
        error_print();
        tls_send_alert(conn, TLS_alert_unexpected_message);
        return -1;
    ...
    if (tls13_record_get_handshake_certificate_verify(
            record, &server_sign_algor, &server_sig, &server_siglen) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_unexpected_message);
        return -1;
    }
    ...
    if (tls13_verify_certificate_verify(
            TLS_server_mode, &server_sign_key, TLS13_SM2_ID,
            TLS13_SM2_ID_LENGTH, &dgst_ctx, server_sig, server_siglen) != 1) {
        error_print();
        return -1;
    }
    digest_update(&dgst_ctx, record + 5, recordlen - 5);
    ...
    tls13_compute_verify_data(server_handshake_traffic_secret, &dgst_ctx,
                              verify_data, &verify_data_len);
    ...
    tls13_record_trace(stderr, record, recordlen, 0, 0);
    if (tls13_record_get_handshake_finished(record, &server_verify_data,
    ...
    /* [12] */ tls13_derive_secret(master_secret, "s ap traffic", &dgst_ctx,
                                   server_application_traffic_secret);
    // generate client_application_traffic_secret
    /* [11] */ tls13_derive_secret(master_secret, "c ap traffic", &dgst_ctx,
    ...
    if (conn->client_certs_len) {
        ...
        if (tls13_record_set_handshake_certificate(
                record, &recordlen, NULL, 0,  // certificate_request_context
        ...
        if (tls13_record_encrypt(&conn->client_write_key, conn->client_write_iv,
                                 conn->client_seq_num, record, recordlen,
                                 padding_len, enced_record,
                                 &enced_recordlen) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_internal_error);
            return -1;
        ...
        tls13_sign_certificate_verify(TLS_client_mode, &conn->sign_key,
                                      TLS13_SM2_ID, TLS13_SM2_ID_LENGTH,
                                      &dgst_ctx, sig, &siglen);
        if (tls13_record_set_handshake_certificate_verify(
                record, &recordlen, client_sign_algor, sig, siglen) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_internal_error);
            return -1;
        }
    ...
    tls13_compute_verify_data(client_handshake_traffic_secret, &dgst_ctx,
                              verify_data, &verify_data_len);
    if (tls_record_set_handshake_finished(record, &recordlen, verify_data,
                                          verify_data_len) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_internal_error);
        goto end;
    ...
    tls13_hkdf_expand_label(digest, server_application_traffic_secret, "key",
                            NULL, 0, 16, server_write_key);
    block_cipher_set_encrypt_key(&conn->server_write_key, cipher,
                                 server_write_key);
    tls13_hkdf_expand_label(digest, server_application_traffic_secret, "iv",
                            NULL, 0, 12, conn->server_write_iv);
    memset(conn->server_seq_num, 0, 8);
    /*
    ...
    */
    ...
    tls13_hkdf_expand_label(digest, client_application_traffic_secret, "key",
                            NULL, 0, 16, client_write_key);
    tls13_hkdf_expand_label(digest, client_application_traffic_secret, "iv",
                            NULL, 0, 12, conn->client_write_iv);
    block_cipher_set_encrypt_key(&conn->client_write_key, cipher,
                                 client_write_key);
    memset(conn->client_seq_num, 0, 8);
...
int tls13_do_accept(TLS_CONNECT *conn) {
    ...
    int client_verify = 0;
    if (conn->ca_certs_len) client_verify = 1;
    ...
    if (tls_record_get_handshake_client_hello(
            record, &protocol, &random, &session_id,
            &session_id_len,  // 不支持SessionID，不做任何处理
            &client_ciphers, &client_ciphers_len, &client_exts,
            &client_exts_len) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_unexpected_message);
        return -1;
    ...
    if (tls_cipher_suites_select(
            client_ciphers, client_ciphers_len, server_ciphers,
            sizeof(server_ciphers) / sizeof(int), &conn->cipher_suite) != 1) {
        error_print();
    ...
    tls13_cipher_suite_get(conn->cipher_suite, &digest,
                           &cipher);  // 这个函数是否应该放到tls_里面？
    digest_init(&dgst_ctx, digest);
    null_dgst_ctx =
        dgst_ctx;  // 在密钥导出函数中可能输入的消息为空，因此需要一个空的dgst_ctx，这里不对了，应该在tls13_derive_secret里面直接支持NULL！
    digest_update(&dgst_ctx, record + 5, recordlen - 5);
    ...
    if (tls13_process_client_hello_exts(
            client_exts, client_exts_len, &server_ecdhe, &client_ecdhe_public,
    ...
    if (tls_record_set_handshake_server_hello(
            record, &recordlen, TLS_protocol_tls12, server_random, NULL,
            0,  // openssl的兼容模式在ClientHello中发送SessionID并检查在ServerHello是否返回，用`-no_middlebox`可关闭兼容模式
    ...
    */
    ...
    if (tls13_record_encrypt(&conn->server_write_key, conn->server_write_iv,
                             conn->server_seq_num, record, recordlen,
                             padding_len, enced_record,
                             &enced_recordlen) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_internal_error);
    ...
    tls_trace("send {Certificate}\n");
    if (tls13_record_set_handshake_certificate(record, &recordlen, NULL, 0,
                                               conn->server_certs,
                                               conn->server_certs_len) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_internal_error);
    ...
    tls_trace("send {CertificateVerify}\n");
    tls13_sign_certificate_verify(TLS_server_mode, &conn->sign_key,
                                  TLS13_SM2_ID, TLS13_SM2_ID_LENGTH, &dgst_ctx,
                                  sig, &siglen);
    if (tls13_record_set_handshake_certificate_verify(
            record, &recordlen, TLS_sig_sm2sig_sm3, sig, siglen) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_internal_error);
        goto end;
    }
    ...
    tls13_compute_verify_data(server_handshake_traffic_secret, &dgst_ctx,
                              verify_data, &verify_data_len);
    if (tls13_record_set_handshake_finished(record, &recordlen, verify_data,
                                            verify_data_len) != 1) {
        error_print();
        tls_send_alert(conn, TLS_alert_internal_error);
        goto end;
    ...
    /* 11 */ tls13_derive_secret(master_secret, "c ap traffic", &dgst_ctx,
                                 client_application_traffic_secret);
    // 因为后面还要解密握手消息，因此client application key, iv
    // 等到握手结束之后再更新
    ...
    if (client_verify) {
        ...
        if (tls13_record_decrypt(&conn->client_write_key, conn->client_write_iv,
                                 conn->client_seq_num, enced_record,
                                 enced_recordlen, record, &recordlen) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_bad_record_mac);
            return -1;
        ...
        if (tls13_record_get_handshake_certificate(
                record, &request_context, &request_context_len, &cert_list,
                &cert_list_len) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_unexpected_message);
            return -1;
        ...
        if (tls13_process_certificate_list(cert_list, cert_list_len,
                                           conn->client_certs,
                                           &conn->client_certs_len) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_unexpected_message);
            return -1;
        }
        if (x509_certs_get_cert_by_index(conn->client_certs,
                                         conn->client_certs_len, 0, &cert,
                                         &certlen) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_unexpected_message);
            return -1;
        }
        if (x509_cert_get_subject_public_key(cert, certlen, &client_sign_key) !=
            1) {
            error_print();
            tls_send_alert(conn, TLS_alert_unexpected_message);
            return -1;
    ...
    if (client_verify) {
        ...
        if (tls13_record_get_handshake_certificate_verify(
                record, &client_sign_algor, &client_sig, &client_siglen) != 1) {
            error_print();
            tls_send_alert(conn, TLS_alert_unexpected_message);
            return -1;
        }
        if (tls13_verify_certificate_verify(TLS_client_mode, &client_sign_key,
                                            TLS13_SM2_ID, TLS13_SM2_ID_LENGTH,
                                            &dgst_ctx, client_sig,
                                            client_siglen) != 1) {
            error_print();
    ...
    tls13_record_trace(stderr, record, recordlen, 0, 0);
    if (tls13_record_get_handshake_finished(record, &client_verify_data,
    ...
    */
    ...
    format_print(stderr, 0, 0, "\n");
    */
    fprintf(stderr, "%s:%d:%s(Connection Established):\n", __FILE__, __LINE__,
            __func__);
